...
const el = {
  ...
  authLoadingScreen: document.querySelector("#authLoadingScreen"),
...
const authUi = createAuthUiState({
  app: el.app,
  loginScreen: el.loginScreen,
  loadingScreen: el.authLoadingScreen
});
...
async function boot() {
  ...
  const configResult = await runConfigBoot({
    loadPublicConfig: () => requestDuringAuthBootstrap(
      "/api/config/public",
      "ใช้เวลานานเกินไปในการโหลดข้อมูลเริ่มต้น กรุณาลองใหม่อีกครั้ง"
    ),
    ...
    showInitializing: showAuthLoading
  });
...
async function restoreSession() {
  ...
  const result = await requestDuringAuthBootstrap(
    "/api/auth/me",
    "ไม่สามารถตรวจสอบการเข้าสู่ระบบได้ภายในเวลาที่กำหนด กรุณาเข้าสู่ระบบใหม่"
  );
  if (!result.ok) {
...
async function logout() {
  await completeLogout({
    requestLogout: () => api("/api/auth/logout", {}),
    clearSession: clearAuthenticatedRuntime,
    clearSavedView: clearSavedActiveView,
    showLogin: () => {
      el.userDropdown?.classList.add("hidden");
...
async function loadAuthenticatedConfig() {
  ...
  const result = await establishAuthenticatedConfig({
    token,
    loadConfig: () => requestDuringAuthBootstrap(
      "/api/config",
      "ไม่สามารถโหลดการตั้งค่าบัญชีได้ภายในเวลาที่กำหนด กรุณาเข้าสู่ระบบใหม่"
    ),
...
async function enterApp() {
  ...
  const restoredView = resolveRestoredView({
    savedView: readSavedActiveView(),
    allowedViews: allowedViewIds(),
    fallbackView: firstView
  });
...
async function requestDuringAuthBootstrap(url, timeoutMessage) {
  try {
    return await withAbortTimeout(
      ({ signal }) => api(url, undefined, { signal }),
      { timeoutMs: AUTH_BOOT_TIMEOUT_MS, message: timeoutMessage }
    );
  } catch (error) {
...
async function api(url, body, { authToken = state.authToken, signal } = {}) {
  const requestGeneration = authenticatedRuntime.getGeneration();
